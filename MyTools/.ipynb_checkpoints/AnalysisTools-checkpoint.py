...
class Snapshot:
    ...
    def masses(self,iPartType=0): #In Sun Masses [M ☉]
        return 1e10/self.h*self.dic[('PartType%d'%iPartType,'Masses')]
    def coords(self,iPartType=0): #In [kpc] =[3.086e+16 km]
        return (self.dic[('PartType%d'%iPartType,'Coordinates')] - self.center)/self.h*self.a
    def vs(self): #In [km/s]
        return self.dic[('PartType0','Velocities')]*self.a**0.5 - self.center_velocity
    def Ts(self): #In [K]
    ...
    def rhos(self): #In [gr/cm^3]
        return ((un.Msun/un.kpc**3).to('g/cm**3') *
                self.dic[('PartType0','Density')] * 1e10 /self.h * self.h**3 / self.a**3)
    ...
    def neutralHydrogenAbundance(self,iPartType=0):
        return self.dic[('PartType%d'%iPartType,'NeutralHydrogenAbundance')]
    def SmoothingLength(self,iPartType=0):
    ...
    def Metallicity(self,iPartType=0):
